...
public:
	...
	void CopyHistory ( value_type* dest, size_t max_amount, size_t& amount, time_type time_range = std::numeric_limits<time_type>::max (), time_type now = std::numeric_limits<time_type>::max () )
	{
		inner_type const * const cp_current ( m_current );
		amount = 0;
